...
RUN_DIR="$LOCAL_SHADOW/stage2/$STAMP"
COMPARE_DIR="$RUN_DIR/compare"
FETCH_DIR="$RUN_DIR/fetched"
LOCAL_LOG="$RUN_DIR/stage2-shadow.log"
REMOTE_RUN_DIR="$REMOTE_SHADOW_RUNS/stage2/$STAMP"
REMOTE_REPO="$REMOTE_RUN_DIR/repo"
LOCAL_REFERENCE_REPO="$RUN_DIR/local-repo"
PATH_MANIFEST="$ROOT/scripts/nas/inputs/stage-2.paths"
...
LOCAL_REFERENCE_COMMAND="node scripts/generate_meta_dashboard_data.mjs"
...
if [[ "$STATUS" -ne 0 ]]; then
  ...
  write_run_metrics "stage2" "$STAMP" "$RUN_DIR" "$STATUS" "$GATE" "$BEFORE_CHECKPOINT" "$AFTER_CHECKPOINT" "$TOTAL_DURATION_SEC" "$LOCAL_REFERENCE_DURATION_SEC" "$NAS_DURATION_SEC" "$MANIFEST_DURATION_SEC" >/dev/null
...
set +e
"${SSH_CMD[@]}" "$NAS_HOST" "set -euo pipefail; . '$NAS_ROOT/tooling/env.sh'; cd '$REMOTE_REPO'; node scripts/generate_meta_dashboard_data.mjs" > "$RUN_DIR/remote.stdout.log" 2> "$RUN_DIR/remote.stderr.log"
RUN_STATUS="$?"
set -e
...
set +e
rsync_from_remote "$REMOTE_REPO/public/dashboard_v6_meta_data.json" "$FETCH_DIR/" >/dev/null
FETCH_STATUS="$?"
set -e
...
set +e
node "$ROOT/scripts/nas/compare-json.mjs" \
  --left "$LOCAL_REFERENCE_REPO/public/dashboard_v6_meta_data.json" \
  --right "$FETCH_DIR/dashboard_v6_meta_data.json" \
  --report "$COMPARE_DIR/dashboard_v6_meta_data.compare.json" | tee -a "$LOCAL_LOG"
