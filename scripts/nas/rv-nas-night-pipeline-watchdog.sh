...
python3 - "$PIPELINE_ROOT" "$OUT_JSON" "$JOURNAL_DIR" "$MIN_RATE_ASSETS_PER_MIN" "$STALE_PROGRESS_MIN" <<'PY'
import glob
import json
import os
import sys
...
pipeline_root, out_json, journal_dir, min_rate_raw, stale_min_raw = sys.argv[1:6]
