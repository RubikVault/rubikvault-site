...
build_search_rebuild_cmd() {
  local mode="${1:-global_registry}"
  local scope_file="public/data/universe/v7/ssot/assets.global.canonical.ids.json"
  if [[ "$mode" == "index_core" ]]; then
    cat <<EOF
node scripts/universe-v7/rebuild-search-exact-from-registry.mjs --scope-file="$scope_file" \
  && node scripts/universe-v7/regenerate-search-local.mjs --scope-file="$scope_file"
EOF
  else
    cat <<EOF
node scripts/universe-v7/rebuild-search-exact-from-registry.mjs \
  && node scripts/universe-v7/regenerate-search-local.mjs
EOF
  fi
}
...
step_command() {
  case "$1" in
    ...
    market_data_refresh)
      ...
      local bulk_min_rows_matched="${RV_EODHD_BULK_MIN_ROWS_MATCHED:-1000}"
    ...
    quantlab_daily_report)
      printf '%s\n' "node scripts/quantlab/build_quantlab_v4_daily_report.mjs && node scripts/ops/build-quantlab-model-coverage.mjs --target-market-date '$TARGET_MARKET_DATE'"
      ;;
    breakout_v12)
      ...
      ;;
    scientific_summary)
      printf '%s\n' "node scripts/universe-v7/build-marketphase-deep-summary.mjs --as-of '$TARGET_MARKET_DATE' --id-mode symbol --scope-file public/data/universe/v7/ssot/assets.global.canonical.ids.json --rows-file mirrors/universe-v7/ssot/assets.global.rows.json --max-symbols '${RV_MARKETPHASE_DEEP_MAX_SYMBOLS:-7000}' && SCIENTIFIC_SCOPE_FILE='public/data/universe/v7/ssot/assets.global.canonical.ids.json' SCIENTIFIC_ROWS_FILE='mirrors/universe-v7/ssot/assets.global.rows.json' node scripts/scientific-analyzer/generate-analysis.mjs && node scripts/build-scientific-summary.mjs && node scripts/ops/build-scientific-per-asset-projection.mjs --target-market-date '$TARGET_MARKET_DATE'"
      ;;
    forecast_daily)
