...
export function updateStatus(repoRoot, status) {
    const statusPath = path.join(repoRoot, PUBLIC_BASE, 'system/status.json');
    ...
    fs.writeFileSync(statusPath, JSON.stringify(statusDoc, null, 2));
...
export function updateLatest(repoRoot, latest) {
    const latestPath = path.join(repoRoot, PUBLIC_BASE, 'latest.json');
...
export function updateLastGood(repoRoot, lastGood) {
    const lastGoodPath = path.join(repoRoot, PUBLIC_BASE, 'system/last_good.json');
    ensureDir(lastGoodPath);
    ...
    fs.writeFileSync(lastGoodPath, JSON.stringify(doc, null, 2));
