...
async function fetchFeed(feed) {
  return withRetries(async () => {
    const res = await fetch(feed.url, { headers: { "User-Agent": "RubikVault/1.0" } });
    if (!res.ok) throw new Error(`rss_http_${res.status}`);
    const text = await res.text();
    if (text.trim().startsWith("<!DOCTYPE") || text.trim().startsWith("<html")) {
      throw new Error("rss_html");
    }
    return text;
...
async function buildNews() {
  ...
  const prev = loadMirror(path.resolve(__dirname, "../public/mirrors/news.json"));
...
function buildEmptyEvent(id, explain) {
  const prev = loadMirror(path.resolve(__dirname, `../public/mirrors/${id}.json`));
  return buildBaseMirror({
    mirrorId: id,
    mode: "EMPTY",
    cadence: "daily",
