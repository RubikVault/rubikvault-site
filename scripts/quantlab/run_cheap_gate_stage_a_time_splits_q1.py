...
from scripts.quantlab.q1_common import (  # noqa: E402
    ...
    infer_panel_output_tag_from_part_glob,
    utc_now_iso,
)
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--candidate-profile",
        choices=["core8", "core16", "core24"],
        default="core24",
        help="core8 keeps the compact baseline set; core16/core24 add progressively richer risk-aware variants.",
    )
    p.add_argument("--run-id-suffix", default="")
    p.add_argument("--fixed-universe-path", help="Path to a JSON file containing the habitat-stratified perfect universe.")
...
def _candidates(profile: str) -> list[Candidate]:
    core8 = [
    ...
    if profile == "core8":
        return core8
    # Extra variants increase candidate diversity while keeping Stage-B gates unchanged.
    core16_extra = [
        Candidate("TSMOM", "tsmom_20_riskadj", lambda: 0.55 * pl.col("z_ret_20d") + 0.25 * pl.col("z_macd_hist") - 0.20 * pl.col("z_vol_20")),
        Candidate("TSMOM", "tsmom_trend_quality", lambda: 0.50 * pl.col("trend_gate") + 0.35 * pl.col("z_ret_20d") + 0.15 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_20_trend_liq", lambda: 0.55 * pl.col("z_ret_20d") + 0.25 * pl.col("trend_gate") + 0.20 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_20_macd_liq", lambda: 0.50 * pl.col("z_ret_20d") + 0.25 * pl.col("z_macd_hist") + 0.25 * pl.col("z_liq")),
        Candidate("MEANREV", "mr_rsi_trendfilter", lambda: -0.70 * pl.col("z_rsi_14") - 0.30 * pl.col("trend_gate")),
        Candidate("MEANREV", "mr_boll_vol", lambda: -0.60 * pl.col("z_boll_z_20") - 0.40 * pl.col("z_vol_20")),
        Candidate("BREAKOUT", "breakout_trend_volfilter", lambda: 0.50 * pl.col("trend_gate") + 0.35 * pl.col("z_macd_hist") - 0.15 * pl.col("z_vol_20")),
        Candidate("QUALITY", "quality_liq_lowvol_macd", lambda: 0.45 * pl.col("z_liq") - 0.35 * pl.col("z_vol_20") + 0.20 * pl.col("z_macd_hist")),
    ]
    core16 = core8 + core16_extra
    if profile == "core16":
        return core16
    core24_extra = [
        Candidate("TSMOM", "tsmom_trend_quality_v2", lambda: 0.42 * pl.col("trend_gate") + 0.38 * pl.col("z_ret_20d") + 0.20 * pl.col("z_liq")),
        Candidate("TSMOM", "tsmom_trend_macd_lowvol", lambda: 0.40 * pl.col("trend_gate") + 0.30 * pl.col("z_macd_hist") + 0.20 * pl.col("z_ret_20d") - 0.10 * pl.col("z_vol_20")),
        Candidate("TSMOM", "tsmom_ret_macd_liq", lambda: 0.45 * pl.col("z_ret_20d") + 0.30 * pl.col("z_macd_hist") + 0.25 * pl.col("z_liq")),
        Candidate("TSMOM", "tsmom_trend_quality_v3", lambda: 0.45 * pl.col("trend_gate") + 0.30 * pl.col("z_ret_20d") + 0.15 * pl.col("z_liq") + 0.10 * pl.col("z_macd_hist")),
        Candidate("TSMOM", "tsmom_trend_defensive", lambda: 0.45 * pl.col("trend_gate") + 0.25 * pl.col("z_ret_20d") + 0.20 * pl.col("z_macd_hist") + 0.20 * pl.col("z_liq") - 0.10 * pl.col("z_vol_20")),
        Candidate("CSMOM", "csmom_trend_liq_v2", lambda: 0.42 * pl.col("trend_gate") + 0.33 * pl.col("z_ret_20d") + 0.25 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_trend_macd_liq", lambda: 0.35 * pl.col("trend_gate") + 0.30 * pl.col("z_ret_20d") + 0.20 * pl.col("z_macd_hist") + 0.15 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_ret5_ret20_liq", lambda: 0.45 * pl.col("z_ret_20d") + 0.25 * pl.col("z_ret_5d") + 0.30 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_20_trend_liq_soft", lambda: 0.63 * pl.col("z_ret_20d") + 0.11 * pl.col("trend_gate") + 0.26 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_ret5_trend_liq_soft", lambda: 0.49 * pl.col("z_ret_20d") + 0.15 * pl.col("z_ret_5d") + 0.10 * pl.col("trend_gate") + 0.26 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_ret5_macd_liq_soft", lambda: 0.47 * pl.col("z_ret_20d") + 0.15 * pl.col("z_ret_5d") + 0.10 * pl.col("z_macd_hist") + 0.28 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_trend_macd_liq_v2", lambda: 0.40 * pl.col("trend_gate") + 0.22 * pl.col("z_ret_20d") + 0.18 * pl.col("z_macd_hist") + 0.20 * pl.col("z_liq")),
        Candidate("CSMOM", "csmom_trend_macd_liq_v3", lambda: 0.46 * pl.col("trend_gate") + 0.20 * pl.col("z_ret_20d") + 0.14 * pl.col("z_macd_hist") + 0.20 * pl.col("z_liq")),
        Candidate("QUALITY", "quality_trend_liq_lowvol", lambda: 0.35 * pl.col("trend_gate") + 0.35 * pl.col("z_liq") - 0.30 * pl.col("z_vol_20")),
        Candidate("BREAKOUT", "breakout_trend_macd_liq", lambda: 0.45 * pl.col("trend_gate") + 0.35 * pl.col("z_macd_hist") + 0.20 * pl.col("z_liq")),
        Candidate("BREAKOUT", "breakout_trend_macd_liq_v2", lambda: 0.50 * pl.col("trend_gate") + 0.30 * pl.col("z_macd_hist") + 0.10 * pl.col("z_liq") - 0.10 * pl.col("z_vol_20")),
        Candidate("BREAKOUT", "breakout_trend_macd_liq_v3", lambda: 0.55 * pl.col("trend_gate") + 0.25 * pl.col("z_macd_hist") + 0.20 * pl.col("z_liq")),
    ]
    return core16 + core24_extra
...
def main(argv: Iterable[str]) -> int:
    ...
    if args.fixed_universe_path:
        upath = Path(args.fixed_universe_path)
        if upath.exists():
            uobj = json.loads(upath.read_text())
            fixed_ids = set()
            for h in uobj.get("habitats", {}).values():
                fixed_ids.update(h)
                
            original_n = len(allow_ids)
            allow_ids = allow_ids.intersection(fixed_ids)
            print(f"[INFO] Applied fixed universe filter: {original_n} -> {len(allow_ids)} (Whitelist: {len(fixed_ids)})")
        else:
            print(f"[WARN] Fixed universe path {args.fixed_universe_path} not found. Skipping filter.")
    ...
    candidates = _candidates(args.candidate_profile)
    ...
    panel_output_tag = str(args.panel_output_tag or infer_panel_output_tag_from_part_glob(args.part_glob) or args.part_glob)
    run_id_suffix = str(args.run_id_suffix or "").strip()
    if not run_id_suffix:
        run_id_suffix = build_stagea_artifact_suffix(
            panel_output_tag=panel_output_tag,
            ...
            profile_payload={
                ...
                "panel_output_tag": panel_output_tag,
                "panel_asof_days": int(args.panel_asof_days),
                "top_liquid_n": int(args.top_liquid_n),
                "fold_count": int(args.fold_count),
    ...
    run_id = build_stagea_time_splits_run_id(args.asof_end_date, run_suffix=run_id_suffix)
