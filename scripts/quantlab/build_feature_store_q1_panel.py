...
def _scan_and_build_panel(
    ...
    vwap_20 = (
        (pl.col("close_raw") * pl.col("volume_raw")).rolling_sum(20).over("asset_id")
        / pl.when(vol_sum_20 > 0).then(vol_sum_20).otherwise(None)
    )
    ...
    feat = (
        ...
        .with_columns(
            [
                ...
                ((pl.col("close_raw") / vwap_20) - 1.0).alias("dist_vwap_20"),
        ...
        .with_columns(
            [
                ...
                pl.col("logret_1d").pow(2).ewm_mean(alpha=1.0 / 20.0, adjust=False).over("asset_id").sqrt().alias("ewma_vol_20"),
                pl.col("logret_1d").pow(2).ewm_mean(alpha=1.0 / 60.0, adjust=False).over("asset_id").sqrt().alias("ewma_vol_60"),
            ]
        )
        .with_columns([pl.col("ewma_vol_20").rolling_std(20).over("asset_id").alias("vov_20")])
        .with_columns(
            [
