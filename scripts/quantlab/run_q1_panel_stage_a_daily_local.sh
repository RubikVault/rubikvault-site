...
if [[ -z "$SNAPSHOT_ID" ]]; then
  if [[ "$AUTO_REFRESH_SNAPSHOT" == "1" ]]; then
    ...
    STEP2_OUTPUT_SUFFIX="${Q1_DAILY_STEP2_OUTPUT_SUFFIX:-q1step2bars_dailyauto}"
    if [[ -f "$STEP1_SCRIPT" && -f "$STEP2_SCRIPT" ]]; then
      echo "[q1-daily-local] auto_refresh_snapshot=1 asof_date=$AUTO_REFRESH_ASOF_DATE asset_classes=$ASSET_CLASSES"
      set +e
      STEP1_OUTPUT="$("$PYTHON_BIN" "$STEP1_SCRIPT" --target-root "$QUANT_ROOT" --asof-date "$AUTO_REFRESH_ASOF_DATE" --include-asset-classes "$ASSET_CLASSES" --snapshot-name "$STEP1_SNAPSHOT_NAME" 2>&1)"
      STEP1_RC=$?
      set -e
      printf '%s\n' "$STEP1_OUTPUT"
      if [[ "$STEP1_RC" -eq 0 ]]; then
        STEP1_SNAPSHOT_ID="$(
          printf '%s\n' "$STEP1_OUTPUT" | "$PYTHON_BIN" -c 'import json, sys; lines=[line.strip() for line in sys.stdin.read().splitlines() if line.strip()]; payload=json.loads(lines[-1]) if lines else {}; print(str(payload.get("snapshot_id") or ""))'
        )"
        if [[ -n "$STEP1_SNAPSHOT_ID" ]]; then
          set +e
          STEP2_OUTPUT="$("$PYTHON_BIN" "$STEP2_SCRIPT" --quant-root "$QUANT_ROOT" --snapshot-id "$STEP1_SNAPSHOT_ID" --include-asset-classes "$ASSET_CLASSES" --output-suffix "$STEP2_OUTPUT_SUFFIX" 2>&1)"
          STEP2_RC=$?
          set -e
          printf '%s\n' "$STEP2_OUTPUT"
          if [[ "$STEP2_RC" -eq 0 ]]; then
            STEP2_SNAPSHOT_ID="$(printf '%s\n' "$STEP2_OUTPUT" | awk -F= '/^snapshot_id=/{print $2}' | tail -n 1)"
            if [[ -n "$STEP2_SNAPSHOT_ID" ]]; then
...
STAGEB_POST_GATE_SOFT_RC="${Q1_DAILY_STAGEB_POST_GATE_SOFT_RC:-1}"
...
PY
  ...
  if [[ -n "$STAGEA_RUN_ID" && -f "$STAGEB_Q1_SCRIPT" ]]; then
    ...
    if [[ "$STAGEB_Q1_RC" -ne 0 ]]; then
      if [[ "$STAGEB_Q1_RC" -eq 41 && "$STAGEB_POST_GATE_SOFT_RC" == "1" ]]; then
        echo "[q1-daily-local] stage_b_q1 soft_fail_exit_code=41 (post-gate/no strict survivor); keeping overall daily run green" | tee -a "$LOG_FILE"
      else
        RC="$STAGEB_Q1_RC"
      fi
    fi
