...
def main(argv: Iterable[str]) -> int:
    ...
    folds_observed_total = (
        int(fold_metrics.select(pl.col("fold_id").n_unique().alias("n")).to_dicts()[0].get("n", 0))
        if fold_metrics.height > 0
        else 0
    )
    # Keep prep fold feasibility aligned with Stage-B light: if the built fold plan
    # overstates what is actually observable on the current panel, do not zero out the
    # entire prep shortlist by demanding unavailable folds.
    folds_effective_total = max(1, min(int(fold_count_built or 1), int(folds_observed_total or fold_count_built or 1)))
    strict_cfg = {
        "folds_used_min": int(folds_effective_total),
        "folds_built_total": int(fold_count_built),
