...
if (!fs.existsSync(MIRROR_ROOT)) {
  console.log(`validate-mirrors: skipped (missing ${MIRROR_ROOT})`);
  process.exit(0);
}
...
// Only validate mirrors that are actually generated by this workflow
// For event-mirrors workflow, only validate event-related mirrors
const EVENT_MIRROR_IDS = new Set([
  "news",
  "why-moved",
  "hype-divergence",
  "earnings",
...
// Check if we're in event-mirrors context (via env var or script name)
const isEventMirrorsContext = process.env.VALIDATE_EVENT_MIRRORS_ONLY === "1" || 
  process.argv[1]?.includes("validate-mirrors");
...
for (const mirrorId of mirrorIds) {
    // Skip validation for mirrors not generated by event-mirrors workflow
    if (isEventMirrorsContext && !EVENT_MIRROR_IDS.has(mirrorId)) {
      continue;
    }
    ...
    if (items.length === 0 && !continuousEmptySoftAllow.has(mirrorId)) {
      const payload = { mirrorId, warning: "empty_items", count: items.length };
