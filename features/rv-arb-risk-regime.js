  ...
  if (!envelope?.ok) {
    ...
    root.innerHTML = `
      <div class="rv-native-error">
        ARB Risk Regime konnte nicht geladen werden.<br />
  ...
  root.innerHTML = `
    ...
    <div class="rv-native-grid rv-compact">
      ...
      <div class="rv-native-kpi">
        ...
        <div class="value">${formatNumber(item?.score0to100, { maximumFractionDigits: 0 })}</div>
      </div>
      <div class="rv-native-kpi">
        <div class="label">VIX Proxy</div>
        <div class="value">${formatNumber(item?.vixValue, { maximumFractionDigits: 2 })}</div>
      </div>
    </div>
    <ul class="rv-native-list">
      ${(explain || []).map((line) => `<li>${line}</li>`).join("")}
    </ul>
    ${formatMetaLines({ meta, envelope })}
  `;
...
async function loadData({ featureId, traceId, logger }) {
  return fetchJSON("/arb-risk-regime", { feature: featureId, traceId, logger });
}

export async function init(root, context = {}) {
  const { featureId = "rv-arb-risk-regime", traceId, logger } = context;
  const data = await getOrFetch("rv-arb-risk-regime", () => loadData({ featureId, traceId, logger }), {
    ttlMs: 30 * 60 * 1000,
    featureId,
    logger
  });
