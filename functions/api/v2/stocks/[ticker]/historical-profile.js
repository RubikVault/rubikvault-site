...
import { fetchStockHistoricalProfile } from '../../../_shared/data-interface.js';
...
export async function onRequestGet(context) {
  ...
  const endpointId = 'v2_historical_profile';
  ...
  if (!ticker) {
    ...
    const envelope = errorEnvelope(
      ...
      { provider: 'v2-historical-profile', data_date: todayUtc, status: 'error', version: 'v2' }
    );
  ...
  const result = await fetchStockHistoricalProfile(ticker, env, request);
